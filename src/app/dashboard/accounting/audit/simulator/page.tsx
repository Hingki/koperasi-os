...
import { simulateRetailLock, simulatePpobLock, simulateRetailSettlement } from '@/app/actions/simulator';
...
export default function SimulatorPage() {
  ...
  const handleSimulateRetailSettlement = async () => {
    setLoading(true);
    setResult(null);
    try {
      const data = await simulateRetailSettlement(amount, cogs);
      setResult(data);
    } catch (error: any) {
      toast({
        title: "Simulation Failed",
            ...
            <Tabs defaultValue="retail" className="w-full">
              ...
              <TabsContent value="retail" className="space-y-4">
                ...
                <div className="flex gap-2">
                  <Button className="flex-1" onClick={handleSimulateRetail} disabled={loading || amount <= 0}>
                    {loading ? '...' : 'Simulate Lock'}
                  </Button>
                </div>

                <div className="pt-4 border-t space-y-2">
                  <label className="text-sm font-medium">Cost of Goods Sold (HPP)</label>
                  <Input
                    type="number"
                    value={cogs}
                    onChange={(e) => setCogs(Number(e.target.value))}
                    min={0}
                    placeholder="Untuk simulasi Settlement"
                  />
                  <Button variant="secondary" className="w-full" onClick={handleSimulateRetailSettlement} disabled={loading || amount <= 0 || cogs <= 0}>
                    {loading ? '...' : 'Simulate Settlement (Revenue)'}
                  </Button>
                </div>
              </TabsContent>
