...
export class ReportService {
    ...
    async getReportData(koperasiId: string, startDate: Date, endDate: Date, unitId?: string) {
        ...
        if (unitId) {
            accountsQuery = accountsQuery.eq('unit_usaha_id', unitId);
        }
        ...
        // Note: For Balance Sheet (as of date), we technically need ALL entries up to endDate.
        // For Income Statement (period), we need entries between start and end.
        // This method is generic, so we'll fetch based on the range provided.
        // If caller wants "As Of", they should provide startDate = '1970-01-01' or similar?
        // Or we just fetch range.

        const { data: entries, error: entriesError } = await this.supabase
            .from('ledger_entry')
            .select('account_debit, account_credit, amount, book_date')
            .eq('koperasi_id', koperasiId)
            .gte('book_date', startDateStr)
            .lte('book_date', endDateStr)
            .eq('status', 'posted'); // Ensure only posted entries are used
    ...
    async getReportDataAsOf(koperasiId: string, asOfDate: Date, unitId?: string) {
        ...
        // 2. Fetch Snapshot (Optimization)
        // Find latest period that has a snapshot and started before or on asOfDate
        const { data: snapshotPeriod } = await this.supabase
            .from('accounting_period')
            .select('id, start_date')
            .eq('koperasi_id', koperasiId)
            .lte('start_date', asOfDateStr)
            .order('start_date', { ascending: false })
            .limit(1)
            .maybeSingle();

        let startDateStr = '1970-01-01'; // Default full history
        ...
        if (snapshotPeriod) {
            const { data: snaps } = await this.supabase
                ...
                .eq('period_id', snapshotPeriod.id);

            if (snaps && snaps.length > 0) {
                startDateStr = snapshotPeriod.start_date;
                snaps.forEach(s => {
                    initialBalances[s.account_id] = Number(s.balance);
                });
            }
        }

        // 3. Fetch Entries from startDateStr
        const { data: entries, error: entriesError } = await this.supabase
            .from('ledger_entry')
            .select('account_debit, account_credit, amount')
            .eq('koperasi_id', koperasiId)
            .lte('book_date', asOfDateStr)
            .gte('book_date', startDateStr)
            .eq('status', 'posted');

        if (entriesError) throw entriesError;

        return { accounts: accounts || [], entries: entries || [], initialBalances };
    }
    ...
    private async validatePeriod(koperasiId: string, date: Date, type: 'final' | 'draft') {
        if (type === 'draft') return; // Draft always allowed
        ...
        const { data: period, error } = await this.supabase
            ...
            .maybeSingle(); // Use maybeSingle to avoid error if no period

        if (!period) {
            // If no period found, we cannot verify it is closed.
            // Requirement: "menolak periode terbuka jika diminta final"
            // Implicitly open if not defined/closed.
            throw new Error('Accounting Period not found for this date. Cannot generate Final Report.');
        }

        if (period.status !== 'closed') {
            throw new Error('Accounting Period is Open. Cannot generate Final Report. Please close the period first.');
        }
    }
    ...
    async getBalanceSheet(koperasiId: string, asOfDate: Date, unitId?: string, isFinal: boolean = false): Promise<BalanceSheetReport> {
        if (isFinal) {
            await this.validatePeriod(koperasiId, asOfDate, 'final');
        }
