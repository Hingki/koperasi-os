...
import { getMemberSavingsBalanceAction } from '@/lib/actions/savings';
...
import { CheckoutDialog } from './pos/checkout-dialog';
...
export default function POSInterface({ initialProducts, members, user }: POSInterfaceProps) {
  ...
  // Effect: Fetch Points and Savings when member selected
  useEffect(() => {
    if (selectedMember) {
      getMemberPointsAction(selectedMember.id).then(setPointsBalance);
      getMemberSavingsBalanceAction(selectedMember.id).then(setSavingsBalance);
    } else {
      setPointsBalance(0);
      setSavingsBalance(0);
      setUsePoints(false);
  ...
  return (
    <div className="flex h-full flex-col md:flex-row bg-slate-100">
            ...
            <div className="flex items-center justify-between bg-red-50 p-2 rounded-lg border border-red-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700 font-medium">Saldo: Rp {savingsBalance.toLocaleString('id-ID')}</p>
                </div>
              </div>
      ...
      <CheckoutDialog
        ...
        koperasiId={user.user_metadata.koperasi_id}
        onSuccess={() => {
          setCart([]);
          setDiscountInput('');
          setVoucherCode('');
      ...
      {scanNotification && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-lg flex items-center gap-3 animate-in slide-in-from-top-4 fade-in duration-300 ${scanNotification.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
          }`}>
