...
function runFix(dryRun = true) {
  ...
  for (const f of files) {
    ...
    // Match CREATE [UNIQUE] INDEX [CONCURRENTLY] name ON ...  -> insert IF NOT EXISTS after INDEX
    const regex = /(create\s+(?:unique\s+)?index\s+(?:concurrently\s+)?)(?!if\s+not\s+exists)/ig;
    if (regex.test(content)) {
      const newContent = content.replace(regex, (m) => m + 'IF NOT EXISTS ');
      if (newContent !== content) {
        did = true;
        if (dryRun) {
          console.log(`Would add IF NOT EXISTS to CREATE INDEX in ${f.name}`);
        } else {
          fs.copyFileSync(f.path, f.path + '.bak');
          fs.writeFileSync(f.path, newContent, 'utf8');
          console.log(`Applied CREATE INDEX idempotency fix to ${f.name} (backup at ${f.name}.bak)`);
        }
      }
    }
  }
  if (!did) console.log('No CREATE INDEX idempotency fixes needed.');
}
