...
class QueryBuilder {
    ...
    async then(resolve: any, reject: any) {
        try {
            console.log('DEBUG: Query on ' + this.table + ' select=' + this._select);
            await new Promise(r => setTimeout(r, 10));
            ...
            rows = rows.filter((row: any) => {
                for (const f of this.filters) {
                    if (f.type === 'eq') { if (row[f.col] !== f.val) return false; }
                    else if (f.type === 'lt') { if (!(row[f.col] < f.val)) return false; }
                    else if (f.type === 'gt') { if (!(row[f.col] > f.val)) return false; }
                    else if (f.type === 'lte') { if (!(row[f.col] <= f.val)) return false; }
                    else if (f.type === 'gte') { if (!(row[f.col] >= f.val)) return false; }
                    else if (f.type === 'in') { if (!f.val.includes(row[f.col])) return false; }
                    else if (f.type === 'or') {
            ...
            if (this._limit > -1) { rows = rows.slice(0, this._limit); }

            // Handle Joins (Basic Mock)
            if (this.table === 'journals' && this._select.includes('journal_lines(*)')) {
                // console.log('DEBUG: Joining journal_lines');
                const allLines = this.data.journal_lines || [];
                rows = rows.map((journal: any) => {
                    const lines = allLines.filter((l: any) => l.journal_entry_id === journal.id);
                    return { ...journal, journal_lines: lines };
                });
            }

            // Execute Mutation
            if (this._insertData) {
                if (this._insertData.idempotency_key) {
                     const exists = (this.data[this.table] || []).find((r: any) => r.idempotency_key === this._insertData.idempotency_key);
                     if (exists) { return reject({ message: 'duplicate key value violates unique constraint', code: '23505' }); }
                }
                const newItem = { created_at: new Date().toISOString(), ...this._insertData, id: this._insertData.id || ('mock_' + Date.now() + '_' + Math.random()) };
                if (!this.data[this.table]) this.data[this.table] = [];
                this.data[this.table].push(newItem);
                this.db.saveData();
                return resolve({ data: this._single ? newItem : [newItem], error: null });
            }
...
// Use Mock Client
console.log('âš ï¸ RUNNING IN SIMULATION MODE (Mock Database)');
...
async function assertLedgerBalanceIntegrity(koperasiId: string) {
  console.log('\nðŸ” [AUDIT] Checking Ledger Balance Integrity...');
  ...
  // console.log('   [Debug] Balance Lines Count:', balanceLines?.length);
  ...
  if (Math.abs(ledgerBalance - expectedBalance) > 1) {
      console.error('âŒ INTEGRITY FAILURE: Ledger does not match Active Transactions!');
      return false;
  } else {
      console.log('âœ… INTEGRITY PASS: Ledger matches Active Transactions.');
      return true;
  }
}
...
async function runScenarios() {
  ...
  console.log('\nðŸ§ª [SCENARIO 1] PPOB Crash After Journal Lock');
  try {
     ...
     console.error('   âŒ FAILED: Should have thrown');
  } catch (error: any) {
     ...
     if (latest && latest.length > 0) {
            ...
            if (trx.status === 'reversed') {
                console.log('   âœ… SCENARIO 1 PASS: Transaction automatically reversed');
            } else {
                console.error(`   âŒ SCENARIO 1 FAIL: Status is ${trx.status}, expected reversed`);
            }
        } else {
            console.log('   âš ï¸ No transaction found (maybe fail before insert?)');
  ...
  console.log('\nðŸ§ª [SCENARIO 2] Double Submit Checkout');
  try {
      ...
      if (successes === 2) { const r1 = (results[0] as any).value; const r2 = (results[1] as any).value; if (r1.transaction.id === r2.transaction.id) console.log('    Double Submit Prevented (Idempotent: Same Transaction Returned)'); else console.warn('    Double Submit Allowed (Different IDs returned)'); }
      
  } catch (e) { console.error(e); }

  // SCENARIO 3: Server Crash
  console.log('\nðŸ§ª [SCENARIO 3] Server Crash Simulation');
  try {
      ...
      if (recTrx && recTrx.status === 'recovered_reversed') {
          console.log('   âœ… SCENARIO 3 PASS: Auto-reversed on restart');
      } else {
          console.error(`   âŒ SCENARIO 3 FAIL: Status ${recTrx?.status}`);
      }
  } catch (e: any) { console.error(e); }

  // SCENARIO 4: Double Settlement
  console.log('\nðŸ§ª [SCENARIO 4] Double Settlement');
  try {
      ...
      if (res && res.status === 'settled') {
          console.log('   âœ… SCENARIO 4 PASS: Idempotent - Returns settled transaction without error');
      } else {
          console.error(`   âŒ SCENARIO 4 FAIL: Unexpected result`, res);
      }
  } catch (error: any) {
      console.error(`   âŒ SCENARIO 4 FAIL: Unexpected error: ${error.message}`);
  }

  // SCENARIO 5: Day End with Stuck Transactions
  console.log('\nðŸ§ª [SCENARIO 5] Day End Stuck Transactions');
  try {
      ...
      if (resLocked?.status === 'recovered_reversed' && resFulfilled?.status === 'recovered_settled') {
          console.log('   âœ… SCENARIO 5 PASS: All stuck transactions recovered correctly');
          console.log(`      - Locked -> Reversed`);
          console.log(`      - Fulfilled -> Settled`);
      } else {
          console.error('   âŒ SCENARIO 5 FAIL: Recovery mismatch');
          console.log('      Results:', results);
      }

  } catch (error: any) {
      console.error(`   âŒ SCENARIO 5 FAIL: ${error.message}`);
  }
